...
|---|---|
| macOS 호스트 | ~6~8GB |
| 로컬 LLM (Qwen3, 4bit 상시 구동) | ~20~22GB |
| Hermes Agent + 자동화 프로세스 | ~3~4GB |
| **VM(K8s)에 배분 가능한 예산** | **~30~32GB** |

> ⚠️ **확인 필요**: 리포지토리 README에는 모델이 `Qwen3.6-35B-A3B`로 기재돼 있으나, 이전 대화에서는 `Qwen3-32B`로 확인됨. 실제 구동 모델 기준으로 이 표를 확정할 것 — MoE(A3B) 구조라면 활성 파라미터 기준 메모리 소요 계산이 밀집 모델과 다르다.
...
| 서비스 | 역할 | 배치 노드 |
|---|---|---|
| PostgreSQL (CloudNativePG) | Flowise 메타데이터, 채팅 히스토리 | node-1 (control-plane과 동거, 경량 상태 서비스) |
...
- [ ] Qwen3 정확한 모델/양자화 비트수 확정 (README `Qwen3.6-35B-A3B` vs 기존 확인된 `Qwen3-32B`) → 확정 후 리소스 예산 재계산
- [ ] Postgres 배포 방식: Helm 차트 vs CloudNativePG 오퍼레이터 (README는 CloudNativePG CRD 사용 중)
