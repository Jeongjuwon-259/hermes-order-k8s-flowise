...
| 항목 | 규칙 |
|---|---|
| **네임스페이스** | 프로젝트 네임스페이스와 동일 (예: `gamin`) |
| **프로젝트명** | `Dockerfile_<네임스페이스>_<기능>` 형식 사용 (예: `Dockerfile_gamin_mcp`) |
| **이미지명** | `<네임스>_\<기능>` (예: `gamin_mcp:latest`) |
| **저장 위치** | `5-gitops/argocd-defaults/` 디렉토리에 배치 |
| **예시** | `Dockerfile_gamin_mcp` → `gamin_mcp:latest`, `Dockerfile_gamin_db` → `gamin_db:latest` |
...
docker build -t gamin_mcp:latest -f 5-gitops/argocd-defaults/Dockerfile_gamin_mcp .

# 네임스페이스 + deployment + service 적용
kubectl apply -f 5-gitops/samples/gamin/
```
...
## 4. 우선 배포 대상 — gamin MCP Python 서버
...
- **네임스페이스:** `gamin`
- **프로젝트명:** `mcp` (fully qualified: `gamin-mcp`, 이미지명: `gamin_mcp`)
- **Dockerfile:** `5-gitops/argocd-defaults/Dockerfile_gamin_mcp`
- **인증(gamin auth) 관련 처리는 이번 단계에서 제외** — 추후 해당 소스 자체에서 해결. 이번 단계는 빌드/배포 파이프라인 검증에 집중한다.
