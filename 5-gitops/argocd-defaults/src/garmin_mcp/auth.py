...
def _has_saved_tokens() -> bool:
    ...
    Older garth versions wrote separate oauth1_token.json/oauth2_token.json
    files; newer versions write a single garmin_tokens.json. Accept either.
    """
    token_dir = Path(get_token_dir())
    legacy = (token_dir / "oauth1_token.json").exists() and (token_dir / "oauth2_token.json").exists()
    modern = (token_dir / "garmin_tokens.json").exists()
...
def login_with_credentials(garmin: Garmin) -> bool:
    ...
    try:
        # Passing tokenstore makes login() save the tokens itself on success.
        garmin.login(tokenstore=get_token_dir())
        return True
    except Exception:
        return False
