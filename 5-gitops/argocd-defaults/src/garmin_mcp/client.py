...
class GarminClient:
    ...
    def download_activity_tcx(self, activity_id: int) -> bytes:
        """Download the raw TCX file for an activity (full trackpoint-level data)."""
        dl_fmt = self._garmin.ActivityDownloadFormat.TCX
    ...
    def get_endurance_score(self, start_date: str, end_date: str | None = None) -> dict[str, Any]:
        validate_date(start_date)
        if end_date:
            validate_date(end_date)
        return self._call("get_endurance_score", start_date, end_date)

    def get_hill_score(self, start_date: str, end_date: str | None = None) -> dict[str, Any]:
        validate_date(start_date)
        if end_date:
            validate_date(end_date)
        return self._call("get_hill_score", start_date, end_date)

    def get_running_tolerance(
        self, start_date: str, end_date: str, aggregation: str = "weekly"
    ) -> list[dict[str, Any]]:
        validate_date(start_date)
        validate_date(end_date)
        return self._call("get_running_tolerance", start_date, end_date, aggregation)
